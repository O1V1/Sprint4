...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	tmp := math.Pow(meanSpeedMsec, 2) / (height / cmInM)
	return (walkingCaloriesWeightMultiplier*weight + tmp*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	return (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * float64(weight*duration)
}
